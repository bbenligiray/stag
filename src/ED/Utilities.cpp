...
//    a[i] = 500*(x-y); // This is the correct formula
  ...
  for (int i=0; i<width*height; i++){LImg[i] = (unsigned char)((L[i]-min)*scale);}
...
//  printf("L: min: %4.2lf, max: %4.2lf\n", min, max);
  ...
  scale = 255.0/(max-min);  
  for (int i=0; i<width*height; i++){aImg[i] = (unsigned char)((a[i]-min)*scale);}
...
//  printf("a: min: %4.2lf, max: %4.2lf\n", min, max);
  ...
  scale = 255.0/(max-min);  
  for (int i=0; i<width*height; i++){bImg[i] = (unsigned char)((b[i]-min)*scale);}
...
void MyRGB2LabFast(unsigned char *redImg, unsigned char *greenImg, unsigned char *blueImg, 
  ...
  for (int i=0; i<width*height; i++){
    ...
    aImg[i] = (unsigned char)(500*(x-y) + 128); // This is the correct formula
...
void RGB2Lab(IplImage *rgbImg, IplImage *labImg){
  ...
  for (int i=0; i<height; i++){
    unsigned char *rgbLine = (unsigned char *)(rgbImg->imageData + i*rgbImg->widthStep);
    unsigned char *labLine = (unsigned char *)(labImg->imageData + i*labImg->widthStep);
...
void RGB2LabOpenCV(unsigned char *redImg, unsigned char *greenImg, unsigned char *blueImg, 
  ...
  int index = 0;
  for (int i=0; i<height; i++){
    unsigned char *p = (unsigned char *)(iplImg->imageData + i*iplImg->widthStep);
  ...
  index = 0;
  for (int i=0; i<height; i++){
    unsigned char *p = (unsigned char *)(iplLabImg->imageData + i*iplLabImg->widthStep);
...
void RGB2Luv(unsigned char *redImg, unsigned char *greenImg, unsigned char *blueImg, 
  ...
  for (int i=0; i<width*height; i++){
    if      (u[i]<min) min = u[i];
    else if (u[i]>max) max = u[i];
  } //end-for

  scale = 255.0/(max-min);  
  for (int i=0; i<width*height; i++){uImg[i] = (unsigned char)((u[i]-min)*scale);}
  ...
  scale = 255.0/(max-min);  
  for (int i=0; i<width*height; i++){vImg[i] = (unsigned char)((v[i]-min)*scale);}
...
void RGB2HSL(unsigned char *redImg, unsigned char *greenImg, unsigned char *blueImg, 
  ...
  for (int i=0; i<width*height; i++){
    ...
    if (deltaMax < 1e-10){ // This is a gray, no chroma...
      ...
      //Chromatic data...
      if (L[i] < 0.5) S[i] = deltaMax/(max+min);
      else            S[i] = deltaMax/(2-max-min);
      ...
      if      (fabs(red-max) <=1e-10)  H[i] = deltaB - deltaG;
      else if (fabs(green-max)<=1e-10) H[i] = (1.0/3.0) + deltaR - deltaB;
      else                             H[i] = (2.0/3.0) + deltaG - deltaR;
  ...
  for (int i=0; i<width*height; i++){
    if      (H[i]<min) min = H[i];
    else if (H[i]>max) max = H[i];
  } //end-for

  double scale = 255.0/(max-min);  
  for (int i=0; i<width*height; i++){HImg[i] = (unsigned char)((H[i]-min)*scale);}
  ...
  scale = 255.0/(max-min);  
  for (int i=0; i<width*height; i++){SImg[i] = (unsigned char)((S[i]-min)*scale);}
...
void RGB2YUV(unsigned char *redImg, unsigned char *greenImg, unsigned char *blueImg, 
  ...
  for (int i=0; i<width*height; i++){
    YImg[i] = (unsigned char)(0.299*redImg[i] + 0.587*greenImg[i] + 0.114*blueImg[i]);
    UImg[i] = (unsigned char)(-0.14713*redImg[i] - 0.28886*greenImg[i] + 0.436*blueImg[i]);
    VImg[i] = (unsigned char)(0.615*redImg[i] - 0.51499*greenImg[i] - 0.10001*blueImg[i]);
  } //end-for
} //end-RGB2YUV
...
/// ColorEdgseSegments
///
void ColorEdgeSegments(EdgeMap *map, unsigned char *colorImg, unsigned char *srcImg){
  int width = map->width;
  int height = map->height;
...
//    memset(colorImg, 255, width*height*3);
  ...
  for (int i=0; i<map->noSegments; i++){
    ...
    if (red == 255 && green == 0 && blue == 0) cg.getNextColor(&red, &green, &blue);
...
/// ShowJointPoints
///
void ShowJointPoints(char *file, EdgeMap *map, unsigned char *jointPoints, unsigned char *srcImg){
  int width = map->width;
  int height = map->height;
...
unsigned char *ScaleImage(unsigned char *srcImg, int width, int height, double scale, int *pw, int *ph){
  ...
  for (int i=0; i<width*height; i++) doubleImg->data[i] = (double)srcImg[i];
